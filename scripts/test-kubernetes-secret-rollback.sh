...
case "$*" in
  '-n prod get secret mnema-secrets -o json')
    if [ "${FAKE_PHASE:-saved}" = saved ]; then
      printf '%s\n' '{"apiVersion":"v1","kind":"Secret","metadata":{"name":"mnema-secrets","namespace":"prod","uid":"secret-uid","resourceVersion":"11","labels":{"owner":"mnema"},"annotations":{"mnema.app/generation":"saved"}},"type":"Opaque","data":{"TOKEN":"b2xk"}}'
    else
      printf '{"apiVersion":"v1","kind":"Secret","metadata":{"name":"mnema-secrets","namespace":"prod","uid":"%s","resourceVersion":"12","labels":{"owner":"mnema"},"annotations":{"mnema.app/generation":"candidate"}},"type":"Opaque","data":{"TOKEN":"%s"}}\n' \
...
PATH="$TEST_ROOT/bin:$PATH" FAKE_KUBECTL_LOG="$TEST_ROOT/kubectl.log" \
  FAKE_PHASE=candidate FAKE_REPLACE_PAYLOAD="$replacement" TOKEN=new \
  "$PRESERVE" restore prod mnema-secrets "$snapshot" TOKEN >/dev/null
...
if PATH="$TEST_ROOT/bin:$PATH" FAKE_KUBECTL_LOG="$TEST_ROOT/kubectl.log" \
  FAKE_PHASE=candidate FAKE_VALUE=dW5leHBlY3RlZA== FAKE_REPLACE_PAYLOAD="$replacement" TOKEN=new \
  "$PRESERVE" restore prod mnema-secrets "$snapshot" TOKEN >/dev/null 2>&1; then
  echo 'Secret rollback must reject unexpected candidate data' >&2
  exit 1
fi
if PATH="$TEST_ROOT/bin:$PATH" FAKE_KUBECTL_LOG="$TEST_ROOT/kubectl.log" \
  FAKE_PHASE=candidate FAKE_UID=other-uid FAKE_REPLACE_PAYLOAD="$replacement" TOKEN=new \
  "$PRESERVE" restore prod mnema-secrets "$snapshot" TOKEN >/dev/null 2>&1; then
  echo 'Secret rollback must reject a replaced Secret identity' >&2
  exit 1
fi
if grep -ERq 'b2xk|bmV3|new|old' "$TEST_ROOT"/*.log "$TEST_ROOT"/*.out; then
