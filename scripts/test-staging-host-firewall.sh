...
case "$*" in
  ...
  'get --raw /api/v1/namespaces/observability/services/http:prometheus:9090/proxy/api/v1/targets')
    case "${FAKE_TARGETS:-healthy}" in
      ...
      stale) printf '%s\n' '{"data":{"activeTargets":[{"labels":{"job":"node-exporter"},"health":"up","lastScrape":"1970-01-01T00:01:00Z"},{"labels":{"job":"kubelet"},"health":"up","lastScrape":"1970-01-01T00:01:00Z"},{"labels":{"job":"cadvisor"},"health":"up","lastScrape":"1970-01-01T00:01:00Z"}]}}' ;;
      pre-activation) printf '%s\n' '{"data":{"activeTargets":[{"labels":{"job":"node-exporter"},"health":"up","lastScrape":"1970-01-01T00:01:40.500Z"},{"labels":{"job":"kubelet"},"health":"up","lastScrape":"1970-01-01T00:01:40.500Z"},{"labels":{"job":"cadvisor"},"health":"up","lastScrape":"1970-01-01T00:01:40.500Z"}]}}' ;;
    esac
    ;;
  'get --raw /apis/metrics.k8s.io/v1beta1/nodes')
    case "${FAKE_TARGETS:-healthy}" in
      stale) printf '%s\n' '{"items":[{"timestamp":"1970-01-01T00:01:00Z"}]}' ;;
      pre-activation) printf '%s\n' '{"items":[{"timestamp":"1970-01-01T00:01:40.500Z"}]}' ;;
...
if [ "${FAKE_FAIL_OLD_FORWARD_DELETE:-false}" = true ] && \
  [ "${1:-}" = -w ] && [ "${2:-}" = -D ] && [ "${3:-}" = FORWARD ] && \
  [ "${5:-}" = MNEMA_POD_HOST_BOUNDARY_A ]; then
  exit 1
fi
...
telemetry_line=$(grep -n -m1 'kubectl get --raw /api/v1/namespaces/observability/services/http:prometheus:9090/proxy/api/v1/targets' "$firewall_log" | cut -d: -f1)
old_delete_line=$(grep -n -m1 'iptables -w -D INPUT -j MNEMA_POD_HOST_BOUNDARY_A' "$firewall_log" | cut -d: -f1)
threshold_line=$(grep -n -m1 'date +%s' "$firewall_log" | cut -d: -f1)
if [ -z "$telemetry_line" ] || [ -z "$old_delete_line" ] || [ -z "$threshold_line" ] || \
  [ "$old_delete_line" -ge "$threshold_line" ] || [ "$threshold_line" -ge "$telemetry_line" ]; then
  echo 'the fresh threshold must be captured after stale-hook removal and before verification' >&2
  exit 1
fi

: >"$firewall_log"
if PATH="$TEST_ROOT/bin:$PATH" FAKE_FIREWALL_LOG="$firewall_log" \
  FAKE_KUBECTL_LOG="$firewall_log" FAKE_ACTIVE_CHAIN=A FAKE_TARGETS=pre-activation \
  TELEMETRY_VERIFY_TIMEOUT_SECONDS=0 \
  KUBE_API_SERVER=https://kubernetes.main.example:6443 \
  KUBE_API_ADDRESSES=198.51.100.10,2001:db8::10 \
  MODE=apply "$RECONCILE" >/dev/null 2>&1; then
  echo 'same-second pre-activation telemetry must not prove the candidate boundary' >&2
  exit 1
fi
grep -Fq 'iptables -w -I INPUT 1 -j MNEMA_POD_HOST_BOUNDARY_A' "$firewall_log"
...
telemetry_line=$(grep -n -m1 'kubectl get --raw /api/v1/namespaces/observability/services/http:prometheus:9090/proxy/api/v1/targets' "$firewall_log" | cut -d: -f1)
old_restore_line=$(grep -n -m1 'iptables -w -I INPUT 1 -j MNEMA_POD_HOST_BOUNDARY_A' "$firewall_log" | cut -d: -f1)
candidate_delete_line=$(grep -n -m1 'iptables -w -D INPUT -j MNEMA_POD_HOST_BOUNDARY_B' "$firewall_log" | cut -d: -f1)
if [ -z "$telemetry_line" ] || [ -z "$old_restore_line" ] || \
  [ -z "$candidate_delete_line" ] || [ "$telemetry_line" -ge "$old_restore_line" ] || \
  [ "$old_restore_line" -ge "$candidate_delete_line" ]; then
  echo 'failed telemetry must restore previous hooks before removing the candidate' >&2
  exit 1
fi

: >"$firewall_log"
if PATH="$TEST_ROOT/bin:$PATH" FAKE_FIREWALL_LOG="$firewall_log" \
  FAKE_KUBECTL_LOG="$firewall_log" FAKE_ACTIVE_CHAIN=A \
  FAKE_FAIL_OLD_FORWARD_DELETE=true TELEMETRY_VERIFY_TIMEOUT_SECONDS=0 \
  KUBE_API_SERVER=https://kubernetes.main.example:6443 \
  KUBE_API_ADDRESSES=198.51.100.10,2001:db8::10 \
  MODE=apply "$RECONCILE" >/dev/null 2>&1; then
  echo 'a partial previous-hook removal must fail closed' >&2
  exit 1
fi
grep -Fq 'iptables -w -D INPUT -j MNEMA_POD_HOST_BOUNDARY_A' "$firewall_log"
grep -Fq 'iptables -w -I INPUT 1 -j MNEMA_POD_HOST_BOUNDARY_A' "$firewall_log"
grep -Fq 'iptables -w -D INPUT -j MNEMA_POD_HOST_BOUNDARY_B' "$firewall_log"
if grep -Fq 'kubectl get --raw /api/v1/namespaces/observability/services/http:prometheus:9090/proxy/api/v1/targets' "$firewall_log"; then
  echo 'a partial hook swap must stop before telemetry verification' >&2
  exit 1
fi
