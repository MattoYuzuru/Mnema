...
4. The lifecycle rule must target the exact backup prefix and expire current, non-current and incomplete multipart data. A harmless `.write-boundary` canary proves expected KMS encryption, denied anonymous access and a bucket policy that rejects both unconditional and `If-Match` overwrites below immutable backup-set prefixes. Each sensitive object is then written with explicit `aws:kms`, private ACL and `If-None-Match: *`. The `latest.env` pointer is written last with `If-None-Match` or the ETag returned with the currently downloaded pointer; a concurrent newer/equal snapshot wins and an older job cannot regress it.
...
The marker contains no database data. A failed run may leave only that marker or a private, encrypted partial object set, but it never advances `latest.env`; the configured lifecycle removes abandoned sets. PutObject is deliberately limited to 5,000,000,000 bytes per file. If a future logical dump reaches that ceiling, stop and implement a reviewed conditional multipart uploader rather than silently falling back to a non-atomic high-level copy.
...
1. Provision a dedicated off-host bucket, bucket/prefix-scoped service account, default `aws:kms` encryption, active versioning and an enabled lifecycle rule. Enforce `If-None-Match` on `<prefix>/postgres/*/*` so unconditional and `If-Match` immutable-set writes return `403`; permit `latest.env` writes only with either `If-None-Match` or `If-Match`. Decide the exact retention duration under O-09 and the legacy-data destruction boundary under O-10; do not infer six months.
