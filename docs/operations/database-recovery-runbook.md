...
5. Observe PostgreSQL readiness, restore completion and reconciliation. The recovery job itself uses an `always()` cancellation guard so GitHub continues to step-condition evaluation after a manual cancellation; its cleanup is another `always()` step capped at four minutes, within GitHub's five-minute cancellation shutdown window. Cleanup issues bounded non-waiting deletion for workloads, ConfigMaps and Secrets immediately, then polls only the fixed boundary until every transient resource is absent. Safe evidence is uploaded only after that cleanup attempt, so an artifact-service stall cannot retain the restored database or S3 Secret.
