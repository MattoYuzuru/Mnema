...
1. `mnema-postgres-backup` starts at minute 17 of every hour in `Etc/UTC`; the CronJob forbids overlap and the dump process holds a PostgreSQL advisory lock shared by scheduled and manually cloned Jobs. Publication remains safe after that lock is released because object sets are write-once and `latest.env` advances with a monotonic conditional compare-and-swap.
...
3. The uploader validates the complete local object set, checks the 5 GB single-PutObject ceiling, and verifies the bucket's expected KMS key, active versioning and enabled lifecycle policy before any network write.
4. The lifecycle rule must target the exact backup prefix and expire current, non-current and incomplete multipart data. Before sensitive upload, harmless policy probes use a sibling, valid backup-shaped ID and every current sensitive basename (`database.dump`, reconciliation, capacity, checksums and metadata). For each key, the uploader proves expected KMS encryption, denied anonymous access and `403` rejection of both unconditional and `If-Match` overwrites. Each real object is then written with explicit `aws:kms`, private ACL and `If-None-Match: *`. The `latest.env` pointer is written last with `If-None-Match` or the ETag returned with the currently downloaded pointer; a concurrent newer/equal snapshot wins and an older job cannot regress it.
...
6. The drill records measured RPO and RTO from explicit timestamps. Both database data and the downloaded dump use size-limited `emptyDir` volumes, so deleting and waiting for the fixed StatefulSet and Job removes their backing data with the Pods. The workflow then removes generated Services, ConfigMaps and Secrets and proves those exact resources are absent, while the owner-created namespace, quota, default-deny policies and scoped ServiceAccount remain. Production is never a restore target.
...
| Name | Purpose |
|---|---|
| `PROD_RECOVERY_KUBECONFIG_B64` | Bounded `mnema-recovery` ServiceAccount credential for the fixed restore namespace; it cannot create/delete namespaces or list/read arbitrary Secrets, and may read/delete only the two exact transient drill Secrets for cleanup verification |
...
The uploader deliberately does not hard-code a retention duration. It proves that the exact configured lifecycle policy is enabled, targets `<prefix>/postgres/`, defines positive current/non-current expiration and incomplete-multipart cleanup periods, and requires versioning state `Enabled`; `Suspended`, empty and unexpected states fail before upload. It performs an authenticated metadata read followed by an unsigned read attempt for every policy probe, uploaded object and downloaded object; a backup is rejected unless anonymous access is explicitly denied. The exact endpoint must be `https://storage.yandexcloud.net`, so an Environment value cannot silently redirect credentials or sensitive data to another origin. The effective object duration is included in safe backup evidence. The duration remains governed by O-09 in [owner decisions](../decisions/owner-decisions-2026-08.md): the owner's preference of up to six months is not a lawful default for every data category. Owner decision 2026-08-30 forbids retaining any full v1 snapshot for the greenfield cutover; every object set produced by this runbook is therefore a purge-manifest target before the replacement opens. Disabling versioning/lifecycle, weakening conditional-write enforcement, changing the KMS key, making an object publicly readable or changing the endpoint makes the operation fail closed before sensitive data is published.
...
```text
<prefix>/postgres/<YYYYMMDDTHHMMSSZ-pod-uid>/.write-boundary
...
The marker and sibling policy-probe objects contain only the fixed text `mnema-backup-write-boundary-v1`, never database data. Probe IDs differ from the real backup ID in the final UUID character and are intentionally not valid restore sets. A failed run may leave only probes/a marker or a private, encrypted partial object set, but it never advances `latest.env`; the configured lifecycle removes abandoned sets. PutObject is deliberately limited to 5,000,000,000 bytes per file. If a future logical dump reaches that ceiling, stop and implement a reviewed conditional multipart uploader rather than silently falling back to a non-atomic high-level copy.
...
1. Provision a dedicated off-host bucket, bucket/prefix-scoped service account, default `aws:kms` encryption, active versioning and an enabled lifecycle rule. Render the repository's canonical conditional-write statements with `S3_BUCKET=<bucket> S3_PREFIX=<prefix> python3 scripts/backup/render_bucket_policy.py`. Merge those two statements into the complete least-privilege bucket policy and preview the entire resulting policy before applying it: Yandex replaces the existing policy rather than merging server-side. The immutable resource is exactly `<prefix>/postgres/*/*`, while `latest.env` permits only `If-None-Match` or `If-Match`. Never apply the fragment by itself because a complete policy also needs the selected service-account allows and any owner/console boundary. Decide ordinary v1 retention under O-09; for the greenfield cutover the owner has already chosen deletion of every full legacy set with no retained snapshot.
2. On `yandex`, require healthy production PostgreSQL/Prometheus, the default `local-path` StorageClass and at least 60 GiB free on `/var/lib/rancher/k3s/storage`. The 20 GiB backup, 20 GiB restore download and 20 GiB isolated PostgreSQL `emptyDir` volumes share the host filesystem but are bounded and Pod-scoped.
...
The fixed restore namespace is deliberately retained. Its ResourceQuota, restricted Pod Security labels and public-HTTPS policy excluding private, loopback, link-local and metadata ranges constrain the recovery credential even when no drill is running. The credential may create drill workloads and write transient Secrets there. It may read/delete only `mnema-restore-db-secrets` and `mnema-restore-backup-secrets` so cleanup can prove them absent; it cannot list Secrets, read any other Secret, mutate NetworkPolicies, create/delete namespaces or access `prod`.
...
## Pre-migration backup for ordinary recoverable changes

This section does not apply to the accepted greenfield account-only cutover, which
must not create or retain a full legacy snapshot. For an ordinary recoverable v1
migration:
...
5. Do not reuse this full logical backup as a greenfield cutover artifact. The replacement runbook retains only account-only/fresh-system recovery data and deletes every full v1 set before opening writes.
...
1. Confirm the fixed boundary marker is version 1, the automatically managed `kube-root-ca.crt` ConfigMap contains a CA, and the namespace contains no Pods, Jobs, StatefulSets, Services, PVCs or other mutable ConfigMaps. Do not delete/recreate an unexpected boundary.
...
5. Observe PostgreSQL readiness, restore completion and reconciliation. The workflow detects a terminal failed restore Job within the five-second polling interval and reports an allowlisted error from either the download init container or the restore container instead of waiting for the two-hour deadline. The recovery job uses a job-level `always()` guard so GitHub continues to evaluate unfinished step conditions after cancellation; the long-running wait step is explicitly gated by `success() && !cancelled()`, while cleanup alone remains `always()` and is capped at four minutes within GitHub's five-minute cancellation shutdown window. Cleanup issues bounded non-waiting deletion for workloads, ConfigMaps and Secrets immediately, then polls only the fixed boundary until every transient resource is absent. Safe evidence is uploaded only after that cleanup attempt, so an artifact-service stall cannot retain the restored database or S3 Secret.
6. Confirm the workflow removed only `postgres`, `mnema-postgres-restore`, their Service, two generated ConfigMaps and two generated Secrets, then reported no fixed-name leftovers. Their `emptyDir` backing data disappears with the deleted Pods. The boundary namespace, marker, `kube-root-ca.crt`, quota, NetworkPolicies, ServiceAccount and Role remain.
...
If automatic cleanup fails, inspect exact resource names and the boundary marker. Remove only resources named in `k8s/backup/restore-drill.yaml` and the four generated ConfigMap/Secret names. Never delete the namespace as workflow cleanup and never generalize a command to `prod`.
...
- Yandex Object Storage [conditional uploads](https://yandex.cloud/en/docs/storage/operations/objects/upload), [bucket-policy enforcement](https://yandex.cloud/en/docs/storage/concepts/policy), [versioning](https://yandex.cloud/en/docs/storage/concepts/versioning), [server-side encryption](https://yandex.cloud/en/docs/storage/tutorials/server-side-encryption), [object lifecycle](https://yandex.cloud/en/docs/storage/concepts/lifecycles) and [bucket access](https://yandex.cloud/en/docs/storage/operations/buckets/bucket-availability): conditional write semantics, active version history, exact KMS ownership, retention and denial of anonymous reads.
