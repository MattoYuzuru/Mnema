...
kubectl diff -f k8s/staging/admission.yaml || test $? -eq 1
kubectl apply -f k8s/staging/admission.yaml
...
Before uploading values, load both environment copies from the owner's external source and run `./scripts/verify-environment-secret-separation.py --desired`. It reports only pass/fail and duplicate key names—never values, lengths or hashes. After the first rollout, run the same tool with an admin kubeconfig and `--live`; any forbidden equality is a no-go. Do not place exports or command output in an issue, pull request or shell history.
