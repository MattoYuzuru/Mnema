...
On the single shared k3s node, install the owner-managed persistent host rule before generating any credential. It blocks every cluster Pod CIDR from node listeners except the shared public web ports, Kubernetes-Service traffic identified by its original ClusterIP destination, and the current kube-system metrics-server Pod on kubelet `10250`; it always blocks the exact directly advertised Kubernetes API endpoint. The narrow system exceptions preserve cluster control-plane/metrics behavior without granting them to staging. The systemd service and one-minute timer rediscover their addresses after k3s starts and fail closed while a trusted metrics-server Pod is being refreshed:
...
Stop if the host uses another authoritative firewall manager until the equivalent rules and persistence are reviewed; do not run two managers that overwrite each other. The kubeconfig generator checks the live iptables rules, enabled/active systemd unit, exact API origin, every resident node InternalIP/ExternalIP on `2379`, `2380`, `6443`, `10250`, production Redis, the Kubernetes Service and metadata. It also proves same-namespace, DNS and public HTTPS egress. Any reachable sensitive endpoint or missing persistent rule is a hard no-go. Reboot the node once during owner acceptance and rerun the generator probes before uploading the credential.

Kubernetes defines a `Role` as namespace-scoped and documents both the `create` limitation and LimitRange default injection before quota admission ([RBAC](https://kubernetes.io/docs/reference/access-authn-authz/rbac/), [LimitRange](https://kubernetes.io/docs/concepts/policy/limit-range/)). The quota is sized for the accepted ~113 GiB free-disk envelope: 3 requested CPU, 8 GiB requested memory, 14 GiB memory limit, 20 GiB PVC storage, 12 GiB requested / 40 GiB limited ephemeral storage, plus explicit counts for each writable API-object class. Its 12-Secret ceiling covers `mnema-secrets`, three TLS Secrets, concurrent cert-manager next-private-key Secrets and bounded renewal/recovery headroom. Each container receives 256 MiB requested / 2 GiB limited ephemeral storage by default and cannot exceed 4 GiB. Verify the live kubelet reports `ephemeral-storage` capacity and enforces filesystem accounting; otherwise add a host-level disk/eviction guard before provisioning staging.
...
`POSTGRES_DB`, `POSTGRES_USER` and `POSTGRES_PASSWORD` are bootstrap-only in both environments. Staging MinIO's `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` and `AWS_BUCKET_NAME` are bootstrap-only as well. The generic workflow permits their initial creation but rejects any later value change before mutation because changing container environment does not rotate initialized PostgreSQL/MinIO state. Rotate them only through a separately reviewed two-phase data procedure that first makes the new credential valid at the data service, then updates/restarts clients, proves both paths, and finally revokes the old credential. Ordinary JWT/OAuth/internal-token changes restart the five backend Deployments that consume `mnema-secrets`; the frontend is intentionally excluded.
...
`Main CI` builds each image once, records its registry digest, and renders both environment manifests from those same six digest files. The non-cancellable staging deployment runs first. Production reaches its separate approval only after staging data services and all six application rollouts succeed. The preview combines application-manifest drift with a value-silent comparison of the desired and live application/Grafana Secrets. A secret-only rotation therefore creates the one real deployment record and is reconciled even when the application diff is empty; a corrected secret can also recover a previously failed rollout. The artifact contains the drift boolean plus a keyed HMAC of the exact desired Secret snapshot and release context. It contains no values or unkeyed value hashes. The mutating job verifies the immutable artifact, recomputes that HMAC before cluster access, repeats live drift/validation before mutation, applies the Secret, applies the release manifest, then restarts only the five backend Secret consumers before rollout verification. A desired A→B change during the approval window therefore fails before the kubeconfig is read. Post-deploy behavioral smoke and rollback evidence are owned by #91.
