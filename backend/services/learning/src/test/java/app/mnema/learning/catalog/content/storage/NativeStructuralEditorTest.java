...
class NativeStructuralEditorTest {
    ...
    void insertDeleteAndMovePreserveOpaqueMetadataSpellingAndDirectOldRoots() {
        ...
        var first = editor.apply(original.snapshot(), read(inserted), new NativeStructuralEdit.Insert(id(6), 0));
        ...
        var second = editor.apply(first.snapshot(), read(moved), new NativeStructuralEdit.Move(id(4), id(2), 0));
        exact(second, moved);
        ObjectNode deleted = moved.deepCopy();
        ((ObjectNode) deleted.path("root").path("content").get(0)).withArray("content").remove(0);
        var third = editor.apply(second.snapshot(), read(deleted), new NativeStructuralEdit.Delete(id(4)));
        exact(third, deleted);
    ...
    void supportedLinkParentRetainsFragmentsAndNewOpaqueSubtreeReusesEqualFragmentBranches() {
        ...
        var next = editor.apply(old.snapshot(), read(doc), new NativeStructuralEdit.Insert(id(3), 1));
        exact(next, doc);
        ...
        var inserted = editor.apply(before.snapshot(), read(opaqueDoc), new NativeStructuralEdit.Insert(id(1), 1));
    ...
    void beginningInsertCopiesPathsNotThePreorderSuffixAndRetainsOpaqueSiblingFragments() {
        ...
        var plan = editor.apply(before.snapshot(), read(next), new NativeStructuralEdit.Insert(id(1), 0));
        exact(plan, next);
    ...
    void nativeTenThousandEntriesAndDepthThirtyTwoRemainRepresentableAfterStructuralEdits() {
        ...
        var plan = editor.apply(old.snapshot(), read(next), new NativeStructuralEdit.Insert(id(1), 0));
        exact(plan, next);
        ...
        exact(editor.apply(deep.snapshot(), read(doc), new NativeStructuralEdit.Insert(id(31), 0)), doc);
    }
    ...
    void futureDocumentsAndEditingWithinOpaqueAncestryFailClosed() {
        ...
        assertThatThrownBy(() -> editor.apply(futureSnapshot, read(futureNext), new NativeStructuralEdit.Delete(id(2))))
                .isInstanceOf(NativeStorageFailure.class);
        assertThatThrownBy(() -> editor.apply(futureSnapshot, read(future), new NativeStructuralEdit.Move(id(2), id(1), 0)))
                .isInstanceOf(NativeStorageFailure.class);
        ...
        assertThatThrownBy(() -> editor.apply(old, read(insert), new NativeStructuralEdit.Insert(id(3), 1)))
                .isInstanceOf(NativeStorageFailure.class);
        ...
        assertThatThrownBy(() -> editor.apply(old, read(delete), new NativeStructuralEdit.Delete(id(4))))
                .isInstanceOf(NativeStorageFailure.class);
        ...
        assertThatThrownBy(() -> editor.apply(old, read(moveOut), new NativeStructuralEdit.Move(id(4), id(5), 1)))
                .isInstanceOf(NativeStorageFailure.class);
        ...
        assertThatThrownBy(() -> editor.apply(old, read(moveIn), new NativeStructuralEdit.Move(id(6), id(3), 1)))
                .isInstanceOf(NativeStorageFailure.class);
        ObjectNode direct = doc.deepCopy();
        ((ObjectNode) direct.path("root").path("content").get(0)).withArray("content").add(node(7, "future"));
        assertThatThrownBy(() -> editor.apply(old, read(direct), new NativeStructuralEdit.Insert(id(2), 1)))
                .isInstanceOf(NativeStorageFailure.class);
    }
    ...
    void exactOneMibInputStillFitsAfterDeletingAnotherSubtree() {
        ...
        var plan = editor.apply(old.snapshot(), read(doc), new NativeStructuralEdit.Delete(id(3)));
        exact(plan, doc);
    ...
    void invalidIntentRootMovesCyclesAndUnrelatedMetadataChangesFailClosed() {
        ...
        var old = codec.encode(UUID.randomUUID(), read(doc)).snapshot();
        assertThatThrownBy(() -> editor.apply(old, read(doc), new NativeStructuralEdit.Delete(id(1)))).isInstanceOf(NativeStorageFailure.class);
        assertThatThrownBy(() -> editor.apply(old, read(doc), new NativeStructuralEdit.Move(id(1), id(4), 0))).isInstanceOf(NativeStorageFailure.class);
        assertThatThrownBy(() -> editor.apply(old, read(doc), new NativeStructuralEdit.Move(id(2), id(3), 0))).isInstanceOf(NativeStorageFailure.class);
        assertThatThrownBy(() -> editor.apply(old, read(doc), new NativeStructuralEdit.Insert(id(4), 4))).isInstanceOf(NativeStorageFailure.class);
        assertThatThrownBy(() -> editor.apply(old, read(doc), new NativeStructuralEdit.Delete(id(999)))).isInstanceOf(NativeStorageFailure.class);
        ObjectNode next = doc.deepCopy();
        ((ObjectNode) next.path("root")).withArray("content").remove(1);
        ((ObjectNode) next.path("root")).put("version", 42);
        assertThatThrownBy(() -> editor.apply(old, read(next), new NativeStructuralEdit.Delete(id(4)))).isInstanceOf(NativeStorageFailure.class);
        var noop = editor.apply(old, read(doc), new NativeStructuralEdit.Move(id(4), id(1), 1));
        assertThat(noop.snapshot()).isSameAs(old);
        assertThat(noop.additions()).isEmpty();
        assertThatThrownBy(() -> new NativeStructuralEdit.Insert(id(1), -1)).isInstanceOf(IllegalArgumentException.class);
