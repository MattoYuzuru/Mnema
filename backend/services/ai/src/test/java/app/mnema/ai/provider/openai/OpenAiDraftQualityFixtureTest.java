...
class OpenAiDraftQualityFixtureTest {
    ...
    @Test
    void importGenerateNormalizesNoisySourceTermsBeforeApply() throws Exception {
        List<FixtureCase> fixtures = loadFixtures("/app/mnema/ai/provider/openai/noisy-source-normalization-fixtures.json");
        OpenAiClient openAiClient = mock(OpenAiClient.class);
        CoreApiClient coreApiClient = mock(CoreApiClient.class);
        AiImportContentService importContentService = mock(AiImportContentService.class);
        CardNoveltyService noveltyService = new CardNoveltyService(coreApiClient);
        ...
        when(importContentService.loadSource(any(), any()))
                .thenReturn(new AiImportContentService.ImportSourcePayload(new byte[0], "application/pdf", 64, false));
        when(importContentService.extractText(any(), any(), any()))
                .thenReturn(new AiImportContentService.ImportTextPayload(
                        buildSourceText(fixtures),
                        "application/pdf",
        ...
        when(openAiClient.createResponse(any(), any())).thenAnswer(invocation -> {
            ...
            if ("mnema_source_normalization".equals(formatName)) {
                ...
                for (int i = 0; i < fixtures.size(); i++) {
                    ...
                    card.put("sourceText", fixture.expectedTerm());
                    ObjectNode fields = card.putObject("fields");
                    fields.put("term", fixture.expectedTerm());
                    fields.put("translation", fixture.badTranslation());
                    fields.put("example", "Example " + (i + 1));
            ...
            usage.put("output_tokens", 60);
            usage.putObject("input_tokens_details").put("cached_tokens", 12);
            usage.putObject("output_tokens_details").put("reasoning_tokens", 6);
            return new OpenAiResponseResult(body.toString(), "qwen3:4b", 120, 60, raw);
        });
        ...
        assertThat(result.resultSummary().path("sourceNormalization").path("reviewedItems").asInt()).isEqualTo(fixtures.size());
        assertThat(result.resultSummary().path("sourceNormalization").path("normalizedItems").asInt()).isEqualTo(fixtures.size());
        assertThat(result.resultSummary().path("sourceNormalization").path("extraction").asText()).isEqualTo("ocr");
    ...
    private static List<FixtureCase> loadFixtures() throws Exception {
        return loadFixtures("/app/mnema/ai/provider/openai/draft-quality-fixtures.json");
    }

    private static List<FixtureCase> loadFixtures(String resourcePath) throws Exception {
        try (InputStream input = OpenAiDraftQualityFixtureTest.class.getResourceAsStream(resourcePath)) {
