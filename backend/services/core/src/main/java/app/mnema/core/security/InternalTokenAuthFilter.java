...
public class InternalTokenAuthFilter extends OncePerRequestFilter {
    ...
    private final CoreInternalAuthProps props;

    public InternalTokenAuthFilter(CoreInternalAuthProps props) {
        this.props = props;
    }
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        Jwt jwt = Jwt.withTokenValue(token)
                .header("alg", "none")
                .claim("scope", "core.internal")
                .issuedAt(Instant.now())
                .expiresAt(Instant.now().plus(Duration.ofHours(1)))
                .build();

        var authorities = List.of(new SimpleGrantedAuthority("SCOPE_core.internal"));
